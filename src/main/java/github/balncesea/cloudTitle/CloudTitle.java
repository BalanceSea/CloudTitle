...
public final class CloudTitle extends JavaPlugin {
    private ConfigManager configs; private Database database; private MessageService messages; private EconomyService economy; private TitleService titles; private MenuManager menus;
    ...
    @Override public void onDisable() { if(menus!=null) menus.closeAll(); if(titles!=null) Bukkit.getOnlinePlayers().forEach(titles::unload); if(database!=null) database.close(); }
    public void reloadPlugin(){ menus.closeAll(); configs.reload(); economy.setup(); titles.reapplyAll(); }
